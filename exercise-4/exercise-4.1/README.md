# Exercise 4.1: Build a `/prime` command
...
Create a **`/prime`** command that **loads relevant codebase context into the agent** before planning or implementation. The agent gathers facts (layout, rules, tooling, repo state) and produces a **short report for you**—optimized for scanning, not for editing files.
...
This repo includes **`prime.example.md`** in this folder: a **dense, fullstack-oriented** example (Python API + web app, many doc paths). It shows the *kind* of thoroughness a `prime` can have.

- Use it for **structure and tone**, not as a copy-paste of paths.
- If your project is smaller, your `prime` can be shorter; the non‑negotiable part is **faithfulness to your repo**, not length.
