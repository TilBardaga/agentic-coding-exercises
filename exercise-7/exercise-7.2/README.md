# Exercise 7.2: System review (process improvement)

**Goal:** Create `/execution-report` and `/system-review` commands so you can improve **process** (plan vs reality), not just code.
...
The execution report command (e.g. `commands/execution-report.md` in the project, or the equivalent path for your tool):
...
Now that you understand execution reports, create a command to analyze them.

### Your task

Create a command file (e.g. `commands/system-review.md`) that analyzes an execution report against the original plan.
...
- Suggest specific **Layer 1** updates (project rules such as `AGENTS.md`)
- Generate actionable improvement recommendations

### Example Solution

See `example-solution/system-review.example.md` for a complete example that follows the **Context → Instructions → Workflow → Output** pattern.
...
Save to: `.agents/execution-reports/<short-feature-name>.md` (use a clear, stable filename)
...
- [ ] You've updated at least one Layer 1 asset (e.g. `AGENTS.md`) based on findings
