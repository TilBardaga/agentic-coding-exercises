# Exercise 7.1: Build a code review command

**Goal:** Create a `/code-review` command that finds technical issues in your code.
...
Many teams run an **automated first pass** on pull requests with a dedicated review command. The goal is to surface **real defects** (logic, security, performance), not to restate what the linter already said.

**Key idea:** The review should be **direct** about problems—politeness is less important than catching issues before they ship.
...
Create a command file (e.g. `commands/code-review.md` in the project, or the path your tool uses for reusable commands):
...
A full reference copy lives in `example-solution/code-review.example.md`. Embedded below for convenience:
...
Try running the command in a **fresh** context on the **latest changes** from a recent feature or PIV loop.

If your tool loads commands from disk, save the file and reload or restart so the new command is picked up.
...
**Code review catches bugs in the code. System review ([exercise 7.2](../exercise-7.2/README.md)) improves your workflow.**