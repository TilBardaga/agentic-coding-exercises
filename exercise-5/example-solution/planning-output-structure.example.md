# Example: planning command output structure

**Purpose:** Reference **plan document** shape (Pattern 2: output for an executor agent). Use with **exercise 5**, Part A.

---

## Important note

This is an **example**, not the only valid shape. Your stack may need different sections. The key is **output optimized for agents** implementing without chat history.
...
### Core Project Files
- `AGENTS.md` - Project principles, logging rules, testing requirements, docstring style
...
**For exercise 5, Part A (document creation for agents):**
...
## Key differences from `/prime` (exercise 4)

| Aspect | `/prime` (human) | Plan document (executor agent) |
|--------|------------------|--------------------------------|
...
**Same framework (Input → Process → Output), different optimization.**
